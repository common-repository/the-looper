...
if (isset($_REQUEST['action'])) {
	...
	if ($action == 'edit') {
		$manage_post_type = 'class="nav-link py-3 rounded-0" href="'.admin_url('admin.php?page=the-looper-taxonomies').'"';
		$add_post_type = 'class="nav-link py-3 rounded-0" href="'.admin_url('admin.php?page=the-looper-taxonomies&action=add').'"';
		$edit_post_type = 'class="nav-link py-3 rounded-0 active" href="#tab-3"';
		$export_post_type = 'class="nav-link py-3 rounded-0" href="'.admin_url('admin.php?page=the-looper-taxonomies&action=tools').'"';
	}
	elseif ($action == 'add') {
...
<div class="container-fluid rounded">
	<div class="row row bg-dark py-2 border-bottom border-primary looper-border-sm">
		<div class="col-12 col-lg-12">
			<h1 class="text-white"><span class="font-weight-bold">Custom Taxonomies Settings</span></h1>
		</div>
	</div>
	<div class="row bg-light">
		<div class="col-12 col-lg-12 p-0">
			<ul class="nav nav-tabs ml-0">
				<li class="nav-item">
					<a <?= $manage_post_type;?>>Custom Taxonomies</a>
				</li>
				<li class="nav-item">
					<a <?= $add_post_type;?>>Add Taxonomy</a>
				</li>
				<?php if( isset($_REQUEST['action']) && $action == 'edit' ): ?>
				<li class="nav-item">
					<a <?= $edit_post_type;?>>Edit Taxonomy</a>
				</li>
				<?php endif; ?>
				<li class="nav-item">
